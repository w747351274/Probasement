...
+ (WaitingHud *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	WaitingHud *hud = [[WaitingHud alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	WaitingHud *hud = [WaitingHud HUDForView:view];
	if (hud != nil) {
...
+ (WaitingHud *)HUDForView:(UIView *)view {
	...
	Class hudClass = [WaitingHud class];
	for (UIView *aView in subviews) {
		if ([aView isKindOfClass:hudClass]) {
			hud = (WaitingHud *)aView;
		}
	}
	return hud;
}
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[RoundProgressView class]];
	
	if (mode == ProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[RoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == ProgressHUDModeAnnularDeterminate) {
			[(RoundProgressView *)indicator setAnnular:YES];
		}
	}
